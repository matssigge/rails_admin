...
RailsAdmin::Config::Fields.register_factory do |parent, properties, fields|
  associations = parent.abstract_model.associations.filter { |a| Array(a.foreign_key).include?(properties.name) && %i[belongs_to has_and_belongs_to_many].include?(a.type) }
  associations.each do |association|
    ...
    possible_field_names = if association.polymorphic?
                             ...
                             [:foreign_key]
                           end.flat_map { |k| Array(association.send(k)) }
